...
class ExpirationProgressView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let totalDuration = expirationDate.timeIntervalSince(startDate)
        let remainingDuration = expirationDate.timeIntervalSince(Date())
        let percentage = max(min(remainingDuration / totalDuration, 1), 0) // between 0 and 1
        ...
        //calculate the color based on percentage (green to red)
        let color = UIColor(
        ...
        let endAngle: CGFloat = startAngle + (2 * .pi * CGFloat(percentage))
